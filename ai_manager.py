...
"""
    ...
    # เพิ่มโอกาสให้สูงขึ้น + บังคับบางตัว + debug
    for engagement in picked:
        p_key = get_persona_key(engagement["user"])
        persona = personas[p_key]
        should_send = (
            random.random() < 0.70 or  # 70% โอกาสต่อ comment (สูงพอที่จะเห็นบ่อย)
            p_key in ["Ariel", "NongBank", "NongFern"]  # persona สายมุก ส่งบ่อยเป็นพิเศษ
        )
        if should_send and MEME_GIF_POOL:
            gif_url = random.choice(MEME_GIF_POOL)
            engagement["image"] = gif_url
            print(f"[DEBUG GIF] {persona['name']} ส่ง GIF: {gif_url}")  # ดูใน terminal ว่าส่งจริงไหม
