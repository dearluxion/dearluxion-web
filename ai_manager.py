...
# --- 5. Crypto Analyst (เพิ่มใหม่) ---
def analyze_crypto(coin_name, price, change_24h, trend_data):
    if not is_ready: return "AI กำลังโหลดกระดานเทรด... (API ไม่พร้อม)"
    
    # Prompt สั่งให้ AI วิเคราะห์
    prompt = f"""
    Role: คุณคือ AI ผู้ช่วยเทรดคริปโต (Crypto Analyst) ในปี 2026
    Task: วิเคราะห์เหรียญ {coin_name} จากข้อมูลนี้:
    - ราคาปัจจุบัน: ${price}
    - เปลี่ยนแปลง 24ชม.: {change_24h}%
    - ข้อมูลกราฟย้อนหลัง 5 วัน: {trend_data}

    ให้ตอบกลับมา 2 รูปแบบ (แยกกันชัดเจน):
    
    1. 🧚‍♀️ **มุมมอง Myla (สาย Safe/ให้กำลังใจ):** - เน้นเตือนสติ ให้ระวัง หรือถ้ากราฟเขียวก็ดีใจแบบน่ารักๆ 
       - ห้ามแนะนำการลงทุนแบบจริงจัง เน้นความห่วงใย
       - ลงท้ายด้วยคำคมสั้นๆ เกี่ยวกับการออม
       
    2. 🍸 **มุมมอง Ariel (สาย Degen/นักซิ่ง):**
       - วิเคราะห์แบบตรงไปตรงมา ถ้าแดงบอก "ยับ", ถ้าเขียวบอก "Moon"
       - ใช้ศัพท์วงการคริปโต (เช่น To the moon, HODL, ติดดอย, Paper Hands)
       - ท้าทายให้ใจกล้าๆ หน่อย (แต่ต้องแฝงตลกร้าย)

    Warning: ต้องต่อท้ายข้อความเสมอว่า "นี่เป็นเพียงการคาดเดาของ AI ไม่ใช่คำแนะนำทางการเงิน (NFA)"
    """
    ...
    try:
        ...
        return f"AI เมาประวาน... วิเคราะห์ไม่ได้ ({e})"