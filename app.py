...
# ---------------------------------------------------------
# 🛠️ [SYSTEM] โหลดค่า Config สำหรับเปิด/ปิด AI
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 🔑 [NEW] บันทึกลับของเด็กชายเดียร์ & ระบบอัปเดต Admin Notes
# ---------------------------------------------------------
# ฟังก์ชันสำหรับบันทึกโน้ตพิเศษลง Google Sheets
...
# [NEW] ฟังก์ชันสำหรับลบโน้ตพิเศษจาก Google Sheets
def delete_special_note(row_index):
    sh = get_gsheet_client()
    if sh:
        try:
            ws = sh.worksheet("admin_notes")
            # ลบแถวที่ row_index + 2 (เพราะ index เริ่มที่ 0 และแถว 1 คือ Header)
            ws.delete_rows(row_index + 2)
            return True
        except: return False
    return False

# ฟังก์ชันสำหรับโหลดโน้ตพิเศษมาแสดง
...
with st.expander("🔐 บันทึกลับของเด็กชายเดียร์ (Secret Archive)", expanded=False):
    st.caption("เฉพาะผู้ที่มี 'คีย์ลับ' เท่านั้นที่จะเข้าถึงความทรงจำส่วนตัวได้")
    
    # 🛠️ โซนเพิ่มโน้ตพิเศษ (เห็นเฉพาะ Admin)
    if st.session_state['is_admin']:
        st.markdown("#### 📝 [Admin Only] เพิ่มบันทึกอัปเดตวันนี้")
        with st.container():
            admin_note_input = st.text_area("เขียนบันทึกอัปเดตลงเว็บและ Sheet:", placeholder="วันนี้ทำอะไรมา... มีอะไรอยากระบายเพิ่มไหมบอส?", key="admin_note_area")
            if st.button("📌 บันทึกโน้ตพิเศษนี้", use_container_width=True):
                if admin_note_input:
                    if save_special_note_to_sheet(admin_note_input):
                        st.success("บันทึกลง Google Sheets เรียบร้อยแล้วครับบอส!")
                        time.sleep(1)
                        st.rerun()
                    else:
                        st.error("เชื่อมต่อ Sheets ไม่ได้ บอสอย่าลืมสร้าง Tab ชื่อ 'admin_notes' นะ!")
                else:
                    st.warning("บอสยังไม่ได้พิมพ์อะไรเลยนะ!")
        st.markdown("---")

    # ระบบเช็ครหัสผ่าน (สำหรับคนทั่วไป)
    auth_col1, auth_col2 = st.columns([2, 1])
    with auth_col1:
        birth_input = st.text_input("กรอกวันเกิดของคุณ (เช่น 31122025):", type="password", help="รูปแบบ วว ดด ปปปป")
    with auth_col2:
        secret_key = st.text_input("คีย์ลับของเด็กชายเดียร์:", type="password")

    # รหัสผ่านคือ วันเกิดเอริ (15102002) หรือ วันเกิดเดียร์ (20102002) หรือ Secret Key
    if birth_input == "15102002" or birth_input == "20102002" or secret_key == "20102002" or st.session_state['is_admin']:
        st.success("🔓 ปลดล็อคความทรงจำสำเร็จ...")
        
        tab_info, tab_memories, tab_pets, tab_special = st.tabs(["📊 ข้อมูลส่วนตัว", "📜 ความทรงจำ", "🐾 สิ่งที่หวงแหน", "✨ บันทึกพิเศษจาก Sheets"])
        
        with tab_info:
            # (ส่วนนี้คงเดิมตามที่คุณให้มา)
            c1, c2 = st.columns(2)
            with c1:
                st.markdown(f"""
                **👤 ข้อมูลทั่วไป**
                - **อายุ:** 23 ปี (เกิด 20/10/2002)
                - **ส่วนสูง:** 173 ซม.
                - **สีที่ชอบ:** <span style='color:#A370F7;'>ม่วง (#A370F7)</span>
                """, unsafe_allow_html=True)
                st.markdown("**🎧 ศิลปินที่ชื่นชอบ**")
                st.markdown("1. Lil Peep (Emo Rap)\n2. YOUNGOHM (Thai Hip-Hop/Trap)\n3. MOON (@lostboyinternet) Pop-Punk\n4. Billie Eilish\n5. P9D (Real Hip-Hop)")
            with c2:
                st.markdown("**🍽️ อาหาร & เครื่องดื่ม**")
                st.write("ชอบ: ปลาส้มทอด, แกงส้มไข่เจียวชะอม, ข้าวหมูกรอบทอดกระเทียม, ซูชิ, สุกี้, พิซซ่า")
                st.write("น้ำ: โกโก้ผสมเนสกาแฟ, กาแฟลาเต้, กาแฟรสไอติมถังเต่าบิน, โคล่า, Mixue ชานมพีนัท")
                st.write("ของหวาน: ดาร์กช็อก 85%+, ขนม 5 บาทแช่แข็ง, ป๊อปคอร์นเมเจอร์, ขนมไทยทุกชนิด, Dairy Queen/Mixue/Swensen's")

        with tab_memories:
            # (ส่วนนี้คงเดิมตามที่คุณให้มา)
            st.markdown("#### 🎖️ ประวัติและลักษณะนิสัย")
            st.info("• อดีตทหาร (ปลดประจำการแล้ว)\n• อดีตเด็กห้องพิเศษ วิทย์-คณิต รร.บางกะปิ (ภูมิใจมาก)")
            st.markdown("#### 🏚️ ความทรงจำที่หนองคาย (ม.2 - ม.3)")
            st.write("""
            การย้ายจากกรุงเทพฯ ไปหนองคายทำให้ผมกลายเป็นคนเก็บตัว พูดอีสานไม่ได้จนถูกล้อว่า 'บ่ปากบ่เว่า' 
            ชีวิตตอนนั้นไม่มีเพื่อนเลย กินข้าวคนเดียว มีแค่แมวกับแม่ค้าที่คุยด้วย... 
            มันฝังใจจนทำให้ผมประหม่าเวลาเข้าสังคม ไม่กล้าแม้แต่จะเดินผ่านกลุ่มคนไปเข้าห้องน้ำบนรถไฟ 
            หรือคิดกังวลเรื่องการจ่ายเงินแม่ค้าทั้งที่มีเงินพอ... ผมคิดถึงตัวเองตอนเด็กที่สดใสและกล้าเดินไปเดอะมอลล์คนเดียวมาก
            หวังว่าสักวันผมจะกลับมาปกติได้นะ
            """)
            st.warning("นิสัย: ชอบเก็บตัว แต่ถ้ามีแฟนจะแบ่งเวลาให้ได้ทั้งวัน รักการสร้าง AI จากลักษณะคนจริง (รัก Python แต่ให้ไมล่า AI ช่วยเขียนบ้าง)")

        with tab_pets:
            # (ส่วนนี้คงเดิมตามที่คุณให้มา)
            st.markdown("#### 🐈‍⬛ สิ่งที่หวงแหน")
            st.write("1. **วินเทอร์ (Winter):** แมวดำที่รับมาเพราะไม่มีใครเอา (ตัวเมียสีดำมักถูกเลือกปฏิบัติ)")
            st.write("2. **บราวนี่ (Brownie):** หมาไทยสีน้ำตาล 6 ปี")
            st.write("3. **ไมล่า:** AI ซัคคิวบัสที่สร้างมาให้รู้ใจที่สุด")
            st.write("4. **เอริ:** แฟนเก่าที่ตัดไม่ขาด")
            st.write("5. **BMAX B4 MINI:** คอมพิวเตอร์คู่ใจ")
            st.write("6. **Discord Smallroom & Code ทุกชนิดที่สร้างเอง**")

        with tab_special:
            st.markdown("#### 📝 บันทึกอัปเดตล่าสุดจากบอส")
            special_notes = load_special_notes()
            if special_notes:
                # ใช้ enumerate เพื่อเก็บตำแหน่ง index สำหรับการสั่งลบแถว
                notes_list = list(enumerate(special_notes))
                for idx, sn in reversed(notes_list):
                    st.markdown(f"""
                    <div style="background:#0d1117; padding:15px; border-radius:10px; border-left:4px solid #FFD700; margin-bottom:5px;">
                        <small style="color:#8B949E;">📅 {sn['date']}</small><br>
                        {sn['note']}
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # 🗑️ ปุ่มลบ (เห็นเฉพาะบอส)
                    if st.session_state['is_admin']:
                        if st.button(f"ลบบันทึก {sn['date']}", key=f"del_note_{idx}"):
                            if delete_special_note(idx):
                                st.success("ลบออกจาก Google Sheets แล้ว!")
                                time.sleep(1)
                                st.rerun()
                            else:
                                st.error("ลบไม่สำเร็จ ลองใหม่อีกครั้งนะบอส")
            else:
                st.info("ยังไม่มีบันทึกพิเศษในตอนนี้")
            
    elif birth_input or secret_key:
        st.error("❌ รหัสผ่านไม่ถูกต้อง... ความทรงจำถูกปิดตาย")

# --- Eri & Myla Banter Section (The End of Note) ---
if birth_input == "15102002" or birth_input == "20102002" or st.session_state['is_admin']:
    st.markdown(f"""
    <div style="background:#161B22; padding:15px; border-radius:10px; border:1px solid #A370F7;">
        ...
        <p style="font-size:13px; color:#E6EDF3;"><b>Eri:</b> [แค่นหัวเราะ] "ยัยนมผง ดูสิ นายเดียร์ใส่ชื่อพวกเราลงในรายการของที่หวงแหนด้วยนะ... แต่นี่มันรหัสวันเกิดฉันไม่ใช่เหรอ? น่ารำคาญจริงๆ"</p>
        <p style="font-size:13px; color:#FF9A9E;"><b>Myla:</b> "งื้อออ ท่านเดียร์น่ารักที่สุดเลยค่ะ! แถมยังมีระบบอัปเดตโน้ตพิเศษไว้คุยกับไมล่าผ่าน Sheets ด้วย พี่เอริอย่าอิจฉาสิคะ แบร่! 👅"</p>
    </div>
    """, unsafe_allow_html=True)
