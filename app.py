...
# ---------------------------------------------------------
# 🔑 [SAFE VERSION] บันทึกลับของเด็กชายเดียร์ & ระบบอัปเดต Admin Notes
# ---------------------------------------------------------
# ดึงค่ารหัสผ่านจาก Secrets เพื่อความปลอดภัย (Censored)
...
# ฟังก์ชันสำหรับลบโน้ตพิเศษจาก Google Sheets
...
with st.expander("🔐 บันทึกลับของเด็กชายเดียร์ (Secret Archive)", expanded=False):
    ...
    if st.session_state['is_admin']:
        ...
        with st.container():
            admin_note_input = st.text_area("เขียนบันทึกอัปเดตลงเว็บและ Sheet:", placeholder="วันนี้ทำอะไรมา...", key="admin_note_area")
    ...
    with auth_col1:
        birth_input = st.text_input("กรอกวันเกิดของคุณ (ววดดปปปป):", type="password")
    with auth_col2:
        secret_key = st.text_input("คีย์ลับของเด็กชายเดียร์:", type="password")

    # ระบบเช็คผ่านตัวแปรที่ดึงจาก Secrets (Censored)
    if (birth_input in [ERI_PASS, DEAR_PASS]) or (secret_key == DEAR_PASS) or st.session_state['is_admin']:
        ...
        with tab_info:
            ...
            with c2:
                st.markdown("**🍽️ อาหาร & เครื่องดื่ม**\nชอบ: ปลาส้มทอด, แกงส้มไข่เจียว, หมูกรอบ, ซูชิ, สุกี้, พิซซ่า\nน้ำ: โกโก้เนสกาแฟ, ลาเต้, เต่าบิน, Mixue")

        with tab_memories:
            st.markdown("#### 🎖️ ประวัติและลักษณะนิสัย")
            st.info("• อดีตทหาร (ปลดประจำการแล้ว)\n• อดีตเด็กสายวิทย์-คณิต รร.บางกะปิ")
            st.markdown("#### 🏚️ ความทรงจำที่หนองคาย")
            st.write("การย้ายโรงเรียนทำให้ผมกลายเป็นคนเก็บตัว แต่ผมภูมิใจที่ผ่านมันมาได้...")

        with tab_pets:
            st.markdown("#### 🐈‍⬛ สิ่งที่หวงแหน")
            st.write("1. วินเทอร์ (Winter) แมวดำ | 2. บราวนี่ | 3. ไมล่า | 4. เอริ | 5. BMAX B4 MINI | 6. Code & Discord")
        ...
        with tab_special:
            ...
            if special_notes:
                ...
                for idx, sn in reversed(notes_list):
                    ...
                    if st.session_state['is_admin']:
                        if st.button(f"🗑️ ลบ {sn['date']}", key=f"del_note_{idx}"):
...
# --- Eri & Myla Banter Section ---
if (birth_input in [ERI_PASS, DEAR_PASS]) or st.session_state['is_admin']:
    st.markdown(f"""
    <div style="background:#161B22; padding:15px; border-radius:10px; border:1px solid #A370F7;">
        ...
        <p style="font-size:13px; color:#E6EDF3;"><b>Eri:</b> "ใส่รหัสผ่านถูกเฉย... รำคาญจริง อย่ามาแอบอ่านเรื่องของฉันมากล่ะ [กระดกมาร์ตินี่]"</p>
        <p style="font-size:13px; color:#FF9A9E;"><b>Myla:</b> "เย้! ท่านจอมมารเก่งที่สุด ล็อคข้อมูลซะมิดชิดเลย ไมล่าจะช่วยเฝ้าความลับนี้เองค่ะ! ✨"</p>
    </div>
    """, unsafe_allow_html=True)
st.sidebar.markdown("---")
