...
# ---------------------------------------------------------
# 🔑 [ULTRA SAFE] บันทึกลับของเด็กชายเดียร์ & ระบบจัดการ Admin Notes
# ---------------------------------------------------------
# ดึงค่ารหัสผ่านจาก Secrets
...
with st.expander("🔐 บันทึกลับของเด็กชายเดียร์ (Secret Archive)", expanded=False):
    st.caption("ต้องระบุ 'วันเกิดเดียร์' และ 'คีย์ลับ' ให้ถูกต้องทั้งคู่เพื่อเข้าถึง")
    
    # โซน Admin สำหรับเขียนโน้ต
    ...
    with auth_col1:
        birth_input = st.text_input("วันเกิดของคุณ (เดียร์):", type="password", help="ระบุวันเกิดเจ้าของเว็บ")
    with auth_col2:
        secret_key = st.text_input("คีย์ลับลับ (เอริ):", type="password", help="ระบุรหัสลับจากคนสำคัญ")

    # 🔒 แก้ไขบัค: ต้องถูกทั้งคู่ (Two-Factor Authentication) หรือเป็น Admin
