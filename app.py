...
with st.sidebar.expander("🧚‍♀️ ถาม-ตอบ กับไมล่า (Q&A)", expanded=True):
    ...
    if selected_q == "🤔 อยากโพสต์เรื่องราวบ้างต้องทำไง?":
        ...
        st.info("🧚‍♀️ **ไมล่า:** เชื่อไหมคะว่าเว็บนี้เขียนด้วย Python ล้วนๆ รวมแล้วมากกว่า **59,589 ตัวอักษร** เลยนะ! บอสพิมพ์จนนิ้วล็อกแล้วมั้งเนี่ย 😹27-12-2568 ")
...
# ---------------------------------------------------------
# ✨ Eri Zone (มุมมืดของเอริ) ✨
# ---------------------------------------------------------
with st.sidebar.expander("🍸 มุมมืดของเอริ (Talk with Eri)"):
    ...
    for chat in st.session_state['eri_chat_history'][-3:]:
        if chat['role'] == 'user':
            ...
            st.markdown(f"<div style='background:#161B22; padding:8px; border-radius:5px; border-left:2px solid #6e5494; font-size:12px;'>🍸 <b>Eri:</b> {chat['message']}</div>", unsafe_allow_html=True)
            
    st.markdown("---")

    # 3. ช่องกรอกข้อความ
    user_msg_eri = st.text_input("คุยอะไร? (อย่าเซ้าซี้)", key="input_eri")
    ...
    if st.button("ส่งข้อความ 💬", key="btn_eri"):
        if not ai_available:
            ...
            eri_persona = """
            คุณคือ "เอริ" (Eri) หญิงสาวบุคลิกเย็นชา พูดน้อย ตรึงเครียด มีอดีตที่ลำบาก 
            ...
            - ชอบดื่ม Martini, ฟังเพลง Metal (Baby Metal), ชอบลาบหมู
            - มองว่า User เป็นคนแปลกหน้า ยกเว้นคนที่ชื่อ "Dearluxion" หรือ "เดียร์" ที่จะยอมคุยดีด้วยนิดหน่อย
            - ตอบกลับสั้นๆ ไม่เกิน 2-3 ประโยค
            """
            try:
                full_prompt = f"{eri_persona}\n\nUser: {user_msg_eri}\nEri:"
                with st.spinner("... [จุดบุหรี่สูบรอคิดคำตอบ]"):
                    response = model.generate_content(full_prompt)
                    eri_reply = response.text.strip()
                
                st.session_state['eri_chat_history'].append({'role': 'user', 'message': user_msg_eri})
                st.session_state['eri_chat_history'].append({'role': 'eri', 'message': eri_reply})
                st.rerun()
            except Exception as e:
                st.error(f"เอริไม่อยากคุยตอนนี้ (Error: {e})")
